...
const getPublicSiteUrl = (bindDomain: string) => {
  const normalizedDomain = bindDomain.trim().toLowerCase();
  const protocol = window.location.protocol === 'https:' ? 'https:' : 'http:';

  if (import.meta.env.DEV && window.location.port) {
    return `${protocol}//${normalizedDomain}:${window.location.port}`;
  }
...
const handleSaveEdit = async () => {
  ...
  const normalizedDomain = editingTemplate.value.domain.trim().toLowerCase();
  ...
  try {
    await templateStore.updateGeneratedTemplate(editingTemplate.value.id, {
...
const handleDeleteTemplate = (id: string) => {
  Modal.confirm({
    ...
    async onOk() {
      await templateStore.deleteGeneratedTemplate(id);
...
const handleEditNav = (index: number) => {
  ...
  editingNavItem.value = JSON.parse(
    JSON.stringify(editingTemplate.value.navItems[index]),
  );
      ...
      <div class="templates-list">
        ...
        <div class="list-items">
          ...
          >
            ...
            <div class="item-template">域名: {{ template.domain }}</div>
          </div>
        </div>
      </div>
      ...
      <div v-if="currentTemplate" class="edit-panel">
        <div class="panel-header">
          ...
          <div class="panel-actions">
            <button class="btn btn-primary" @click="handleEditClick">编辑模板</button>
            <button class="btn btn-danger" @click="handleDeleteTemplate(currentTemplate.id)">删除</button>
          </div>
        </div>
        ...
        <div class="panel-content">
          <div class="info-section">
            ...
            <div class="info-grid">
              ...
              <div class="info-item">
                <label>绑定域名</label>
                <p class="site-path">{{ currentTemplate.domain }}</p>
              </div>
              <div class="info-item">
                <label>预览链接</label>
                <a :href="getPublicSiteUrl(currentTemplate.domain)" target="_blank" class="site-link">
                  访问页面 →
                </a>
              </div>
            </div>
      ...
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label>绑定域名</label>
            <input
              v-model="editingTemplate.domain"
              type="text"
              class="form-input"
          ...
          <div class="form-actions">
            ...
            <button class="btn btn-success" :disabled="submitting" @click="handleSaveEdit">
              {{ submitting ? '保存中...' : '保存更改' }}
            </button>
          </div>
        </div>
      ...
      <div class="modal-content modal-small" @click.stop>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <label>图标</label>
            <input
              v-model="editingNavItem.icon"
              type="text"
              placeholder="例如: lucide:home"
              class="form-input"
            />
          </div>
          <div class="form-actions">
